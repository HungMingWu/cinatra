...
namespace cinatra {
	...
	public:
		...
		void start() {
			co_spawn(socket_.get_executor(),
				[self = shared_from_this()]{ return self->do_read(); }, detached);
		}
		...
		awaitable<void> write_chunked_header(std::string_view mime,bool is_range=false) {
			...
			co_await boost::asio::async_write(socket_,
				...
				use_awaitable);
			handle_chunked_header();
		}

		awaitable<void> write_chunked_data(std::string&& buf, bool eof) {
			reset_timer();
	...
	private:
		...
		awaitable<void> async_read_some() {
			auto self = this->shared_from_this();
			try {
				size_t bytes_transferred = co_await socket_.async_read_some(boost::asio::buffer(req_.buffer(), req_.left_size()), use_awaitable);
				co_await self->handle_read(bytes_transferred);
			}
		...
		awaitable<void> handle_read(std::size_t bytes_transferred) {
			//1. read some
			...
			if (ret == parse_status::not_complete) { //4.2 not completed, continue read
				...
				if (req_.has_body()) { //5.1 has body
					...
					co_await handle_header_request(); //do business, deal with the complete request
				}
			}
		}

		awaitable<void> do_read_head() {
			reset_timer();
			auto self = this->shared_from_this();
			try {
				size_t bytes_transferred = co_await socket_.async_read_some(boost::asio::buffer(req_.buffer(), req_.left_size()), use_awaitable);
		...
		awaitable<void> do_read_body() {
			reset_timer();

			auto self = this->shared_from_this();
			try {
				size_t bytes_transferred = co_await boost::asio::async_read(socket_,
					boost::asio::buffer(req_.buffer(), req_.left_body_len()),
					use_awaitable);
		...
		awaitable<void> handle_string_body(std::size_t bytes_transferred) {
		...
		//-------------web socket----------------//
		awaitable<void> response_handshake() {
			std::vector<boost::asio::const_buffer> buffers = res_.to_buffers();
			if (buffers.empty()) {
				close();
				return;
			...
			auto self = this->shared_from_this();
			try {
				co_await boost::asio::async_write(socket_, buffers,
					use_awaitable);
				...
				req_.set_current_size(0);
				co_await do_read_websocket_head(SHORT_HEADER);
			}
			catch (const boost::system::error_code& ec)
			{
				close();
		...
		awaitable<void> do_read_websocket_head(size_t length) {
			auto self = this->shared_from_this();
			try {
				size_t bytes_transferred  = co_await boost::asio::async_read(socket_,
					boost::asio::buffer(req_.buffer(), length),
					use_awaitable);
				...
				if (ret == parse_status::complete) {
					...
					req_.set_current_size(0);
					req_.fit_size();
					co_await do_read_websocket_data(req_.left_body_len());
				}
				else if (ret == parse_status::not_complete) {
					req_.set_current_size(bytes_transferred);
					co_await do_read_websocket_head(ws_.left_header_len());
				}
				else {
					req_.call_event(data_proc_state::data_error);
					close();
		...
		awaitable<void> do_read_websocket_data(size_t length) {
			auto self = this->shared_from_this();
			try {
				size_t bytes_transferred = co_await boost::asio::async_read(socket_,
					boost::asio::buffer(req_.buffer(), length),
